...
objectAssign(Tweet.prototype, {
  ...
  afterCreate: function afterCreate(resolve, reject) {
    Logger.DEBUG(`doing afterCreate with ${this.uploaded_photo}`);

    if (this.uploaded_photo) {
      fs.copyFile(this.uploaded_photo.path, this.photoPath(), fs.constants.COPYFILE_FICLONE, err => {
...
objectAssign(Tweet, {
  create: function create(options) {
    let id = mk_id();
    let nuevo = new Tweet(extend(options, {
      id: id,
      created_at: Date.now()
    }));
    let db_px = collection.put(id, nuevo.value());
    let creat_px = new Promise((resolve, reject) => {
      db_px.catch(err => reject(err)).then(() => nuevo.afterCreate(resolve, reject));
    });
    return creat_px;
  },
  list: function list(before = MIN_ID, limit = 25) {
    let rs = collection.createValueStream({
      gt: before,
      limit: limit
    });
    let got_tweets = [];

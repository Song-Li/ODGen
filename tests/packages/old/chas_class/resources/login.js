...
login.post('/', express.urlencoded({extended: true}), (req, resp) => {
  var candidate_user

  current_user(req, resp).
    then(maybe_user => {
      if (null != maybe_user) {
    ...
    then(scrypt_result => {
      ...
      Logger.ERROR(reason)
      if (! resp.locals.did_render)
        resp.render('login/index')
    })
})

login.post('/new', express.urlencoded({extended: true}), (req, resp) => {
  current_user(req, resp).
    then(maybe_user => {
